(() => {
  ...
  function missionObservation(){
    ...
    return {
      ...
      strategy:{
        ...
        v2:state.strategyV2?{sources:v2,incident:incidentDefs.find(x=>x[0]===state.incident)?.[1]||'',revision:{hypothesis:labelFrom(revisionHypotheses,state.revisionCausal.hypothesis),information:labelFrom(revisionInfoOptions,state.revisionCausal.information),weakness:labelFrom(revisionWeaknesses,state.revisionCausal.weakness)}}:null
      }
    };
  }
  ...
  function renderQualityUsage(){
    ...
    h.innerHTML=Object.entries(groups).map(([data,items])=>`<article class="usage-duel"><div class="usage-data"><span>MÊME DONNÉE</span><strong>${data}</strong></div><div class="usage-sides">${items.map(([id,_d,use])=>{const v=state.qualityUsage[id]||'',r=state.qualityUsageReason[id]||'';return `<section><strong>${use}</strong><div class="use-decision"><button data-use="${id}" data-value="acceptable" class="${v==='acceptable'?'active good':''}">Acceptable</button><button data-use="${id}" data-value="fragile" class="${v==='fragile'?'active warn':''}">Fragile</button></div>${v?`<div class="quality-reason"><span>Dimension la plus décisive</span><div class="choice-grid compact">${qualityOptions.map(([qid,l])=>`<button data-quality-reason="${id}" data-value="${qid}" class="${r===qid?'active':''}">${l}</button>`).join('')}</div></div>`:''}</section>`}).join('')}</div></article>`).join('');
    h.querySelectorAll('[data-use]').forEach(b=>b.onclick=()=>{state.qualityUsage[b.dataset.use]=b.dataset.value;if(!state.qualityUsageReason[b.dataset.use])state.qualityUsageReason[b.dataset.use]='';save();renderQualityUsage();renderSnapshots();renderWorkflowStatuses();syncCompletionUI()});
    h.querySelectorAll('[data-quality-reason]').forEach(b=>b.onclick=()=>{state.qualityUsageReason[b.dataset.qualityReason]=b.dataset.value;save();renderQualityUsage();renderWorkflowStatuses();syncCompletionUI()});
  }
  ...
  function renderSnapshots(){
    ...
    if($('#snapshotRevision')){const inc=incidentDefs.find(x=>x[0]===state.incident),c=state.revisionCausal;$('#snapshotRevision').innerHTML=snapshotHtml('Évolution v1 → v2',[['Incident',inc?.[1]||''],['Hypothèse cassée',labelFrom(revisionHypotheses,c.hypothesis)],['v1',(state.strategyV1?.sources||[]).map(sourceName).join(' · ')],['v2',(state.strategyV2||[]).map(sourceName).join(' · ')],['Information menacée',labelFrom(revisionInfoOptions,c.information)],['Faiblesse restante',labelFrom(revisionWeaknesses,c.weakness)]])}
  }
