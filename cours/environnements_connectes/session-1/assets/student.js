(() => {
  ...
  function renderWorkflowStatuses(){
    ...
    const retDone=retrievalDefs.filter(r=>r.opts.some(([id])=>id===state.retrieval[r.id])).length; setWorkflowStatus('retrievalStatus',`${retDone} / 4 réponses`,retDone===4);
  ...
  $('#finishBtn').onclick=()=>{if(!retrievalDefs.every(r=>r.opts.some(([id])=>id===state.retrieval[r.id])))return toast('Répondez aux quatre questions flash avant d’ouvrir la synthèse.');state.completed=true;save();$('#finalSynthesis').hidden=false;$('#finishBtn').hidden=true;renderStepper()};
