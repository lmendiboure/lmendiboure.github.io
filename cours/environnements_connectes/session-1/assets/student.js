(() => {
  const STORAGE_KEY = 'environnements-connectes-session1-v5';
  ...
  const defaultState = {
    version:5, screen:0, frontier:0, completed:false, teamName:'', conceptUnlocks:{}, stopChallenges:{},
  ...
  function missionObservation(){
    ...
    return {
      ...
      interpretation:classifyDefs.map(([id,text])=>({example:text,kind:labelFrom(classifyOptions,state.classify[id])})),
      ...
      fitnessForUse:qualityUsageDefs.map(([id,_data,use,expected,label])=>({id,data:label,use,assessment:state.qualityUsage[id]||'',reference:expected})),
      strategy:{
        ...
        v2:state.strategyV2?{sources:v2,incident:incidentDefs.find(x=>x[0]===state.incident)?.[1]||'',revision:{hypothesis:labelFrom(revisionHypotheses,state.revisionCausal.hypothesis),action:labelFrom(revisionActions,state.revisionCausal.action),weakness:labelFrom(revisionWeaknesses,state.revisionCausal.weakness)}}:null
      }
    };
  }
  ...
  function renderWorkflowStatuses(){
    ...
    const profDone=profileTargets.filter(id=>state.sourceProfiles[id]?.zone).length;
    setWorkflowStatus('profileStatus',`${profDone} / 4 sources placées`,profDone===4);
    ...
    const obsDone=obsDefs.filter(([id])=>!!state.observationCases[id]).length;
    setWorkflowStatus('observationStatus',`${obsDone} / 3 choix`,obsDone===3);
    const quDone=qualityUsageDefs.filter(([id])=>!!state.qualityUsage[id]).length;
    setWorkflowStatus('qualityUsageStatus',`${quDone} / ${qualityUsageDefs.length} usages`,quDone===qualityUsageDefs.length);
    ...
    const claimsDone=state.strategy.filter(id=>state.strategyClaims[id]?.role&&state.strategyClaims[id]?.limit).length;
    setWorkflowStatus('strategyClaimsStatus',`${claimsDone} / ${state.strategy.length||2} source(s) qualifiée(s)`,state.strategy.length>=2&&claimsDone===state.strategy.length);
    ...
    const c=state.revisionCausal, causalDone=['hypothesis','action','weakness'].filter(k=>!!c[k]).length; setWorkflowStatus('causalStatus',`${causalDone} / 3 repères`,causalDone===3);
  ...
  function gate(screen,index){if(screen===0&&index===1){const ranked=state.ranks.filter(Boolean);if(state.needs.length<5||ranked.length<3||new Set(ranked).size<3||ranked.some(id=>!state.priorityImpacts[id]))return 'Retenez au moins 5 informations, fixez 3 priorités et associez chacune à un enjeu.'}if(screen===2&&index===3&&!decisionDefs.every(d=>(state.decisions[d.id]||[]).length>=1))return 'Choisissez l’information que vous demanderiez en premier pour chacune des quatre situations.';if(screen===3&&index===4&&state.chain.length!==5)return 'Remettez les cinq niveaux dans le bon ordre avant la restitution.';if(screen===5&&index===6&&!profileTargets.every(id=>state.sourceProfiles[id]?.zone))return 'Placez les quatre sources sur la carte avant de continuer.';if(screen===6&&index===7&&!classifyDefs.every(([id])=>state.classify[id]))return 'Rangez les six observations dans les quatre familles.';if(screen===8&&index===9){const core=['variable','unit','where','when','source'];if(!core.every(x=>state.metadata.includes(x))||Object.keys(state.timeMatch).length<3)return 'Reconstituez le contexte essentiel de la valeur et associez les trois temps.'}if(screen===9&&index===10&&!obsDefs.every(([id])=>state.observationCases[id]))return 'Choisissez un dispositif pour chacune des trois situations.';if(screen===11&&index===12&&!qualityUsageDefs.every(([id])=>state.qualityUsage[id]))return 'Prenez position sur les quatre usages avant de continuer.';if(screen===12&&index===13&&!state.strategyV1)return 'Figez une stratégie v1 complète avant la restitution.';if(screen===14&&index===15){const c=state.revisionCausal;if(!state.incident||!state.strategyV2||!c.hypothesis||!c.action||!c.weakness)return 'Choisissez un incident, construisez la v2 et complétez les trois repères de révision.'}return ''}
  ...
  function renderClassify(){
    ...
    bank.innerHTML=classifyDefs.map(([id,text])=>`<button type="button" class="sorting-card ${selected===id?'selected':''} ${state.classify[id]?'placed':''}" data-sort-card="${id}"><strong>${text}</strong><span>${state.classify[id]?labelFrom(classifyOptions,state.classify[id]):'à ranger'}</span></button>`).join('');
    bank.querySelectorAll('[data-sort-card]').forEach(b=>b.onclick=()=>{state.classifySelected=b.dataset.sortCard;save();renderClassify()});
    bins.querySelectorAll('[data-classify-bin]').forEach(bin=>{const v=bin.dataset.classifyBin,host=bin.querySelector('.bin-items'),items=classifyDefs.filter(([id])=>state.classify[id]===v);host.innerHTML=items.map(([id,text])=>`<span>${text}</span>`).join('')||'<em>déposez une carte ici</em>';bin.classList.toggle('ready',!!selected);bin.onclick=()=>{if(!state.classifySelected)return toast('Choisissez d’abord une carte à ranger.');state.classify[state.classifySelected]=v;state.classifySelected='';save();renderClassify();renderSnapshots();syncCompletionUI()}});
    const progress=$('#classifyProgress');if(progress)progress.textContent=`${classifyDefs.filter(([id])=>state.classify[id]).length} / 6 rangées`;
  }
  ...
  function renderQualityUsage(){
    const h=$('#qualityUsageGrid');if(!h)return;const groups={};qualityUsageDefs.forEach(x=>(groups[x[4]]??=[]).push(x));
    h.innerHTML=Object.entries(groups).map(([data,items])=>`<article class="usage-duel"><div class="usage-data"><span>MÊME DONNÉE</span><strong>${data}</strong></div><div class="usage-sides">${items.map(([id,_d,use])=>{const v=state.qualityUsage[id]||'';return `<section><strong>${use}</strong><div class="use-decision"><button data-use="${id}" data-value="acceptable" class="${v==='acceptable'?'active good':''}">Acceptable</button><button data-use="${id}" data-value="fragile" class="${v==='fragile'?'active warn':''}">Fragile</button></div></section>`}).join('')}</div></article>`).join('');
    h.querySelectorAll('[data-use]').forEach(b=>b.onclick=()=>{state.qualityUsage[b.dataset.use]=b.dataset.value;save();renderQualityUsage();renderSnapshots();renderWorkflowStatuses();syncCompletionUI()});
  }
  ...
  function renderCausal(){const h=$('#revisionCausalBuilder');if(!h)return;const c=state.revisionCausal,block=(title,key,defs)=>`<section><strong>${title}</strong><div class="choice-grid compact">${defs.map(([id,l])=>`<button data-causal="${key}" data-value="${id}" class="${c[key]===id?'active':''}">${l}</button>`).join('')}</div></section>`;h.innerHTML=block('1 · Hypothèse devenue fausse','hypothesis',revisionHypotheses)+block('2 · Réponse ajoutée à v2','action',revisionActions)+block('3 · Faiblesse restante','weakness',revisionWeaknesses);h.querySelectorAll('[data-causal]').forEach(b=>b.onclick=()=>{state.revisionCausal[b.dataset.causal]=b.dataset.value;save();renderCausal();renderSnapshots();renderDesign()})}
  ...
  function renderSnapshots(){
    ...
    if($('#snapshotSources'))$('#snapshotSources').innerHTML=snapshotHtml('Ce que vous avez distingué',[['Classification',classifyDefs.map(([id,text])=>`${text} → ${labelFrom(classifyOptions,state.classify[id])}`).join('<br>')]]);
    ...
    if($('#snapshotRevision')){const inc=incidentDefs.find(x=>x[0]===state.incident),c=state.revisionCausal;$('#snapshotRevision').innerHTML=snapshotHtml('Évolution v1 → v2',[['Incident',inc?.[1]||''],['Hypothèse cassée',labelFrom(revisionHypotheses,c.hypothesis)],['v1',(state.strategyV1?.sources||[]).map(sourceName).join(' · ')],['v2',(state.strategyV2||[]).map(sourceName).join(' · ')],['Réponse',labelFrom(revisionActions,c.action)],['Faiblesse restante',labelFrom(revisionWeaknesses,c.weakness)]])}
  }
  ...
  $('#finishBtn').onclick=()=>{if(!retrievalDefs.every(r=>state.retrieval[r.id]))return toast('Répondez aux quatre questions d’ancrage mémoire avant d’ouvrir la synthèse.');state.completed=true;save();$('#finalSynthesis').hidden=false;$('#finishBtn').hidden=true;renderStepper()};
