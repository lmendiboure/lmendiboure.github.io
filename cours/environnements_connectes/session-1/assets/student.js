(() => {
  ...
  const floodChainDefs=[['obs_level','Hauteur d’eau et vitesse d’évolution au point suivi'],['noise_sensor','Installer un capteur connecté'],['info_risk','Savoir si l’accès est menacé dans les prochaines minutes'],['measure_station','1,24 m à 14:02 · +8 cm en 10 min'],['noise_weather','Prévision météo régionale à J+3'],['decision_close','Fermer ou maintenir l’accès'],['noise_dashboard','Afficher un tableau de bord plus lisible'],['phen_hydro','Montée / débordement de la Nive au droit de l’accès']];
  ...
  const timeLabels=[['phenomenon','Temps du phénomène (phenomenon time)'],['result','Temps de production du résultat (result time)'],['arrival','Temps d’arrivée / réception']];
  ...
  const retrievalDefs=[
    {id:'r1',q:'Avant de choisir une source, par quoi faut-il commencer ?',opts:[['decision','Par la décision et l’information nécessaire'],['sensor','Par le capteur disponible']],correct:'decision',why:'Décision d’abord : elle donne sa valeur à l’information.'},
    {id:'r2',q:'Une donnée ancienne est-elle forcément mauvaise ?',opts:[['depends','Non, cela dépend de l’usage et de l’horizon de décision'],['yes','Oui, une donnée ancienne est toujours inutilisable']],correct:'depends',why:'La qualité dépend de l’usage : ancienne ne signifie pas automatiquement inutile.'},
    {id:'r3',q:'Deux sources différentes utilisent le même service amont. Sont-elles indépendantes ?',opts:[['no','Pas complètement : elles partagent une dépendance'],['yes','Oui, puisqu’elles sont différentes']],correct:'no',why:'Des sources différentes peuvent échouer ensemble à cause d’une dépendance commune.'}
  ];
  const conceptDefs={
  ...
  function renderMetadata(){const h=$('#metadataTokens');if(!h)return;const selectable=metadataDefs.filter(([id])=>!['unit','dashboard'].includes(id));h.innerHTML=selectable.map(([id,l])=>`<button class="token ${state.metadata.includes(id)?'active':''}" data-meta="${id}">${l}</button>`).join('');h.querySelectorAll('[data-meta]').forEach(b=>b.onclick=()=>{const id=b.dataset.meta;state.metadata=state.metadata.includes(id)?state.metadata.filter(x=>x!==id):[...state.metadata,id];save();renderMetadata();renderSnapshots()});const labels={variable:'température de l’air',where:'station RIV-03',when:'observé à 14:02',source:'capteur T-17',quality:'±0,3 °C · capteur abrité'};$('#metadataResult').innerHTML=`<strong>Observation reconstituée</strong><span>32 °C</span><span>reçu à 14:30</span>${state.metadata.map(id=>`<span>${labels[id]}</span>`).join('')}`;renderTimeMatch();const c=$('#comparisonReasons');if(c){c.innerHTML=comparisonReasonDefs.map(([id,l])=>`<button class="token ${state.comparisonReasons.includes(id)?'active':''}" data-comp-reason="${id}">${l}</button>`).join('');c.querySelectorAll('[data-comp-reason]').forEach(b=>b.onclick=()=>{const id=b.dataset.compReason;state.comparisonReasons=state.comparisonReasons.includes(id)?state.comparisonReasons.filter(x=>x!==id):[...state.comparisonReasons,id];save();renderMetadata();renderSnapshots()})}renderTimeChallenge()}
