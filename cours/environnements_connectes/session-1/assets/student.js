(() => {
  const STORAGE_KEY = 'environnements-connectes-session1-v7';
  ...
  const defaultState = {
    version:7, screen:0, frontier:0, completed:false, teamName:'', conceptUnlocks:{}, stopChallenges:{},
  ...
  const conceptDefs={
    ...
    chain:{title:'Deux sens pour une même chaîne',bridge:'Le cas de Banca a montré deux mouvements complémentaires : pendant la conception, vous partez de la décision pour descendre vers ce qu’il faut mesurer ; en exploitation, l’information remonte du phénomène observé vers la décision.',formal:'<strong>Concevoir :</strong> décision → information nécessaire → phénomène → observable → mesure. <strong>Exploiter :</strong> phénomène → observable → mesure → information interprétée → décision.',carry:'Si une technologie semble évidente, revenez d’abord au sens conception : <strong>quelle décision, quelle information, quel observable ?</strong>',summary:'Conception : décision → mesure · exploitation : phénomène → décision.',keep:'Ne confondez pas le sens du raisonnement de conception et le sens de circulation de l’information.'},
  ...
  function missionObservation(){
    ...
    return {
      ...
      observationChain:{design:state.floodChain.map(id=>floodChainDefs.find(x=>x[0]===id)?.[1]||id),operation:[...state.chain]},
  ...
  function gate(screen,index){
    ...
    if(screen===3&&index===4){if(state.floodChain.length<5||state.chain.length<5)return 'Complétez une tentative entière sur le cas de Banca puis sur le sens de fonctionnement. Vous n’avez pas besoin d’avoir tout juste pour aller à la restitution.';}
    if(screen===5&&index===6){if(!profileTargets.every(id=>state.sourceProfiles[id]?.zone)||!sourceUseCaseDefs.every(([id])=>state.sourceUseCases?.[id]))return 'Placez les quatre sources puis utilisez la carte sur les trois besoins proposés. Une tentative complète suffit pour poursuivre.'}
    ...
    if(screen===8&&index===9){const core=['variable','unit','where','when','source'];if(!core.every(x=>state.metadata.includes(x))||Object.keys(state.timeMatch).length<3||state.comparisonReasons.length<2)return 'Reconstituez le contexte essentiel, répondez aux trois questions temporelles, puis identifiez au moins deux raisons qui rendent A et B difficiles à comparer. Une tentative complète suffit.'}
  ...
  function renderDecisionChallenge(){
    ...
    if(!chosenInfo){h.innerHTML='<div class="inline-feedback warn">Traitez d’abord la décision CRUE dans l’activité principale : le défi réutilise votre propre choix.</div>';return}
    const chosenLabel=d.options.find(x=>x[0]===chosenInfo)?.[1]||chosenInfo;
    const picked=challengeActions.find(x=>x[0]===c.assumption);
    h.innerHTML=`<div class="challenge-loss"><span>INFORMATION RETIRÉE</span><strong>${chosenLabel}</strong></div><div class="challenge-question"><strong>Que faites-vous maintenant ?</strong><span>Un seul choix. Regardez ensuite l’hypothèse que vous venez de rendre visible.</span></div><div class="challenge-action-grid">${challengeActions.map(([id,l])=>`<button type="button" data-assumption="${id}" class="${c.assumption===id?'active':''}">${l}</button>`).join('')}</div>${picked?`<div class="inline-feedback ok"><b>Hypothèse révélée</b> ${picked[2]}</div>`:''}`;
  ...
  function renderChain(){
    ...
    renderSimpleChain('#floodCaseChain',floodChainDefs,state.floodChain,5,arr=>{state.floodChain=arr;save();renderChain();renderSnapshots();syncCompletionUI()},floodSlotLabels);
    ...
    if(out&&state.chain.length===5){const ok=chainItems.every((id,i)=>state.chain[i]===id);out.insertAdjacentHTML('beforeend',`<div class="inline-feedback ${ok?'ok':'warn'}">${ok?'Oui : en exploitation, le phénomène est observé, une mesure est produite, interprétée en information, puis utilisée pour décider.':'Une étape est encore inversée. En exploitation, partez du phénomène physique et remontez vers la décision. Vous pourrez malgré tout continuer après cette tentative complète.'}</div>`)}
  }
  ...
  function renderForestPair(){
    ...
    state.sourceUseCases=state.sourceUseCases||{};
    h.innerHTML=`<div class="source-usecase-grid">${sourceUseCaseDefs.map(([id,need,expected],i)=>{const picked=state.sourceUseCases[id]||'';return `<article class="source-usecase"><span>CAS ${i+1}</span><strong>${need}</strong><div class="source-usecase-zones">${sourceZones.map(([z,l])=>`<button type="button" data-source-usecase="${id}" data-zone="${z}" class="${picked===z?'active':''}">${l}</button>`).join('')}</div>${picked?`<small class="${picked===expected?'ok':'warn'}">${picked===expected?'Profil cohérent avec le besoin.':'Choix discutable : relisez surtout l’étendue à couvrir et le rythme demandé. Une tentative complète suffit pour poursuivre.'}</small>`:''}</article>`}).join('')}</div>`;
    h.querySelectorAll('[data-source-usecase]').forEach(b=>b.onclick=()=>{state.sourceUseCases[b.dataset.sourceUsecase]=b.dataset.zone;save();renderForestPair();renderWorkflowStatuses();syncCompletionUI()})
  }
  ...
  function renderQualityUsage(){
    ...
    h.innerHTML=Object.entries(groups).map(([data,items])=>{dossierIndex++;const complete=items.every(([id])=>state.qualityUsage[id]&&state.qualityUsageReason[id]);return `<article class="usage-duel ${complete?'duel-complete':''}"><div class="usage-data"><span>DOSSIER ${dossierIndex} · MÊME DONNÉE</span><strong>${data}</strong><small>${qualityScenarioContext[data]||'Deux usages · un seul jeu de données'}</small></div><div class="usage-sides">${items.map(([id,_d,use],i)=>{const v=state.qualityUsage[id]||'',r=state.qualityUsageReason[id]||'';return `<section class="usage-case ${v?'judged':''}"><span class="usage-decision-label">${qualityUsageActors[id]||`DÉCISION ${i+1}`}</span><strong>${use}</strong><div class="use-decision"><button type="button" data-use="${id}" data-value="acceptable" aria-pressed="${v==='acceptable'}" class="${v==='acceptable'?'active good':''}">Acceptable pour cet usage</button><button type="button" data-use="${id}" data-value="fragile" aria-pressed="${v==='fragile'}" class="${v==='fragile'?'active warn':''}">Fragile pour cet usage</button></div><div class="quality-reason ${v?'is-enabled':'is-locked'}"><span>${v?'Quelle dimension explique principalement votre jugement ?':'Prenez d’abord position sur cet usage'}</span><div class="quality-dimension-grid">${qualityOptions.map(([qid,l])=>`<button type="button" data-quality-reason="${id}" data-value="${qid}" ${v?'':'disabled'} aria-pressed="${r===qid}" class="quality-dimension ${r===qid?'active':''}">${l}</button>`).join('')}</div></div></section>`}).join('')}</div>${complete?`<div class="duel-compare-prompt"><b>COMPAREZ LES DEUX DÉCISIONS</b><span>La donnée est identique. Qu’est-ce qui change dans l’usage pour rendre une dimension bloquante ici et acceptable ailleurs ?</span></div>`:''}</article>`}).join('');
  ...
  function renderCausal(){
    const h=$('#revisionCausalBuilder'),r=$('#revisionResidualBuilder'),c=state.revisionCausal,map=incidentCausalMap[state.incident]||{};
    ...
    if(h){h.innerHTML=block('1 · Quelle hypothèse de votre v1 vient de tomber ?','hypothesis',filter(revisionHypotheses,map.hypothesis),'Les choix proposés sont filtrés à partir de l’incident retenu.')+block('2 · Quelle information ne pouvez-vous plus tenir pour acquise ?','information',filter(revisionInfoOptions,map.information),'Choisissez l’information directement affectée avant de chercher une solution.');h.querySelectorAll('[data-causal]').forEach(b=>b.onclick=()=>{state.revisionCausal[b.dataset.causal]=b.dataset.value;save();renderCausal();renderSnapshots();renderDesign();renderWorkflowStatuses();syncCompletionUI()})}
    if(r){r.innerHTML=block('Après v2, qu’est-ce qui reste encore fragile ?','weakness',revisionWeaknesses,'Une bonne révision rend le risque résiduel visible au lieu de prétendre l’annuler.');r.querySelectorAll('[data-causal]').forEach(b=>b.onclick=()=>{state.revisionCausal.weakness=b.dataset.value;save();renderCausal();renderSnapshots();renderDesign();renderWorkflowStatuses();syncCompletionUI()})}
  }
  ...
  function renderSnapshots(){
    ...
    if($('#snapshotChain'))$('#snapshotChain').innerHTML=snapshotHtml('Deux sens de la chaîne',[['Conception · du besoin vers la mesure',state.floodChain.map(id=>esc(floodChainDefs.find(x=>x[0]===id)?.[1]||id)).join(' → ')],['Exploitation · du phénomène vers la décision',state.chain.map(esc).join(' → ')]]);
  ...
  function renderStopRitual(){$$('[data-stop-challenge]').forEach(h=>{const id=h.dataset.stopChallenge,d=stopChallenges[id];if(!d)return;const shown=!!state.stopChallenges[id];h.innerHTML=`<div class="ritual-card challenge"><span class="ritual-kicker">CONTRE-EXEMPLE</span> <strong>${d[0]}</strong>${shown?`<p>${d[1]}</p>`:`<p>Ouvrez ce cas après avoir comparé vos raisonnements en classe.</p><button class="btn soft" data-reveal-challenge="${id}">Voir le contre-exemple</button>`}</div>`});$$('[data-reveal-challenge]').forEach(b=>b.onclick=()=>{state.stopChallenges[b.dataset.revealChallenge]=true;save();renderStopRitual()});$$('[data-concept]').forEach(h=>{const id=h.dataset.concept,d=conceptDefs[id];if(!d)return;const on=!!state.conceptUnlocks[id];h.innerHTML=`<div class="ritual-card unlock"><span class="ritual-kicker">REPÈRE À CONSERVER</span>${on?`<strong>${d.title}</strong><div class="unlock-sequence"><section><span>Ce que vos choix ont montré</span><p>${d.bridge}</p></section><section class="formalise"><span>Règle</span><p>${d.formal}</p></section><section class="carry"><span>À réutiliser ensuite</span><p>${d.carry}</p></section></div>`:`<strong>Quelle règle gardez-vous pour la suite ?</strong><p>Comparez d’abord vos réponses, puis affichez la formulation de référence.</p><button class="btn primary" data-unlock="${id}">Afficher la règle à retenir</button>`}</div>`});$$('[data-unlock]').forEach(b=>b.onclick=()=>{state.conceptUnlocks[b.dataset.unlock]=true;save();renderStopRitual();renderFieldGuide();updateUnlockButtons()});$$('[data-reality]').forEach(r=>r.hidden=!state.conceptUnlocks[r.dataset.reality]);updateUnlockButtons()}
