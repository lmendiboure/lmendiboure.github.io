(() => {
  ...
  function renderWorkflowStatuses(){
    ...
    const quDone=qualityUsageDefs.filter(([id])=>!!state.qualityUsage[id]&&!!state.qualityUsageReason[id]).length;
    setWorkflowStatus('qualityUsageStatus',`${quDone} / ${qualityUsageDefs.length} usages qualifiés`,quDone===qualityUsageDefs.length);
    ...
    {const v1=state.strategyV1?.sources||state.strategy||[],v2=state.strategyV2||[],delta=v1.length!==v2.length||v1.some(id=>!v2.includes(id))||v2.some(id=>!v1.includes(id))||!!state.revisionCausal.action;setWorkflowStatus('revisionStatusUI',delta?'Delta v2 explicité':'v1 → v2 · aucun delta',!!state.incident&&delta)};
    const c=state.revisionCausal, causalDone=['hypothesis','information','weakness'].filter(k=>!!c[k]).length; setWorkflowStatus('causalStatus',`${causalDone} / 3 repères`,causalDone===3);
    ...
    {const v1=state.strategyV1?.sources||state.strategy||[],v2=state.strategyV2||[],delta=v1.length!==v2.length||v1.some(id=>!v2.includes(id))||v2.some(id=>!v1.includes(id))||!!state.revisionCausal.action;setGuidedSubstep('guidedA10Causal',!!state.incident&&delta)};
  }
  function gate(screen,index){if(screen===0&&index===1){const ranked=state.ranks.filter(Boolean);if(state.needs.length<5||ranked.length<3||new Set(ranked).size<3||ranked.some(id=>!state.priorityImpacts[id]))return 'Retenez au moins 5 informations, fixez 3 priorités et associez chacune à un enjeu.'}if(screen===2&&index===3&&!decisionDefs.every(d=>(state.decisions[d.id]||[]).length>=1))return 'Choisissez l’information que vous demanderiez en premier pour chacune des quatre situations.';if(screen===3&&index===4&&state.chain.length!==5)return 'Remettez les cinq niveaux dans le bon ordre avant la restitution.';if(screen===5&&index===6&&(!profileTargets.every(id=>state.sourceProfiles[id]?.zone)||state.forestPair.length!==2||!state.forestComplement))return 'Placez les quatre sources puis choisissez une paire complémentaire et la raison principale de cette complémentarité.';if(screen===6&&index===7&&(!classifyDefs.every(([id])=>state.classify[id])||!state.classifyOrigin))return 'Classez les six situations puis distinguez la provenance de la prévision météo.';if(screen===8&&index===9){const core=['variable','unit','where','when','source'];if(!core.every(x=>state.metadata.includes(x))||Object.keys(state.timeMatch).length<3||state.comparisonReasons.length<2)return 'Reconstituez le contexte essentiel, associez les trois temps puis identifiez au moins deux raisons qui rendent A et B non directement comparables.'}if(screen===9&&index===10&&!obsDefs.every(([id])=>state.observationCases[id]))return 'Choisissez un dispositif pour chacune des trois situations.';if(screen===11&&index===12&&!qualityUsageDefs.every(([id])=>state.qualityUsage[id]&&state.qualityUsageReason[id]))return 'Pour les six usages, prenez position et identifiez la dimension de qualité la plus décisive.';if(screen===12&&index===13&&!state.strategyV1)return 'Figez une stratégie v1 complète avant la restitution.';if(screen===14&&index===15){const c=state.revisionCausal,v1=state.strategyV1?.sources||state.strategy||[],v2=state.strategyV2||[],sourceDelta=v1.length!==v2.length||v1.some(id=>!v2.includes(id))||v2.some(id=>!v1.includes(id)),operationalDelta=!!c.action;if(!state.incident||!state.strategyV2||(!sourceDelta&&!operationalDelta)||!c.hypothesis||!c.information||!c.weakness)return 'Choisissez un incident, produisez au moins un delta réel en v2 (source et/ou règle d’exploitation), puis explicitez hypothèse cassée, information menacée et faiblesse restante.'}return ''}
  ...
  function renderQualityUsage(){
    ...
    h.innerHTML=Object.entries(groups).map(([data,items])=>{const complete=items.every(([id])=>state.qualityUsage[id]&&state.qualityUsageReason[id]);return `<article class="usage-duel ${complete?'duel-complete':''}"><div class="usage-data"><span>MÊME DONNÉE</span> <strong>${data}</strong> <small>Deux usages · un seul jeu de données</small></div><div class="usage-sides">${items.map(([id,_d,use])=>{const v=state.qualityUsage[id]||'',r=state.qualityUsageReason[id]||'';return `<section class="usage-case ${v?'judged':''}"><strong>${use}</strong><div class="use-decision"><button type="button" data-use="${id}" data-value="acceptable" aria-pressed="${v==='acceptable'}" class="${v==='acceptable'?'active good':''}">Acceptable</button><button type="button" data-use="${id}" data-value="fragile" aria-pressed="${v==='fragile'}" class="${v==='fragile'?'active warn':''}">Fragile</button></div><div class="quality-reason ${v?'is-enabled':'is-locked'}"><span>${v?'Quelle dimension pèse le plus ?':'Jugez d’abord cet usage'}</span><div class="quality-dimension-grid">${qualityOptions.map(([qid,l])=>`<button type="button" data-quality-reason="${id}" data-value="${qid}" ${v?'':'disabled'} aria-pressed="${r===qid}" class="quality-dimension ${r===qid?'active':''}">${l}</button>`).join('')}</div></div></section>`}).join('')}</div>${complete?`<div class="duel-compare-prompt"><b>COMPAREZ</b> <span>Qu’est-ce qui a changé entre les deux usages pour faire évoluer votre jugement ou la dimension décisive ?</span></div>`:''}</article>`}).join('');
    h.onclick=e=>{
      const use=e.target.closest('[data-use]');
      if(use&&h.contains(use)){state.qualityUsage[use.dataset.use]=use.dataset.value;if(!state.qualityUsageReason[use.dataset.use])state.qualityUsageReason[use.dataset.use]='';save();renderQualityUsage();renderSnapshots();renderWorkflowStatuses();syncCompletionUI();return}
      const reason=e.target.closest('[data-quality-reason]');
      if(reason&&h.contains(reason)&&!reason.disabled){state.qualityUsageReason[reason.dataset.qualityReason]=reason.dataset.value;save();renderQualityUsage();renderSnapshots();renderWorkflowStatuses();syncCompletionUI()}
    };
  }
  ...
  function renderRevisionStudio(){
    ...
    h.innerHTML=`<article class="revision-card revision-before"><div class="eyebrow">v1 · figée</div><h3>Avant incident</h3><p class="revision-card-help">Votre référence. Elle ne peut plus être réécrite.</p><div class="revision-source-list">${v1.map(id=>`<span><b>V1</b> ${sourceName(id)}</span>`).join('')}</div></article><div class="revision-arrow">→</div><article class="revision-card revision-after"><div class="eyebrow">v2 · modifiable</div><h3>Après incident</h3><p class="revision-card-help">Conservez, retirez ou ajoutez des sources. Vous pouvez aussi modifier la règle d’exploitation.</p><div class="revision-source-picker">${strategyDefs.map(([id,t])=>{const on=v2.includes(id),was=v1.includes(id),status=on?(was?'CONSERVÉE':'AJOUTÉE'):(was?'RETIRÉE':'DISPONIBLE');return `<button type="button" class="revision-source-choice ${on?'active':''} ${was?'was-v1':'candidate'} ${was&&!on?'removed':''}" data-v2="${id}" aria-pressed="${on}"><span>${t}</span><b>${status}</b></button>`}).join('')}</div><div class="revision-rule"><span>RÈGLE D’EXPLOITATION V2 · facultatif si le delta de sources suffit</span><div class="revision-rule-grid">${ruleOptions.map(([id,l])=>`<button type="button" data-v2-rule="${id}" class="${action===id?'active':''}" aria-pressed="${action===id}">${l}</button>`).join('')}</div></div><div class="revision-delta ${added.length||removed.length||action?'has-delta':''}"><b>DELTA V1 → V2</b> <span>${removed.length?`Retiré : ${removed.map(sourceName).join(', ')}. `:''}${added.length?`Ajouté : ${added.map(sourceName).join(', ')}. `:''}${action?`Règle : ${labelFrom(ruleOptions,action)}.`:''}${!added.length&&!removed.length&&!action?'Aucun changement encore explicité.':''}</span></div></article>`;
    h.querySelectorAll('[data-v2]').forEach(b=>b.onclick=()=>{const id=b.dataset.v2,arr=[...(state.strategyV2||v1)];state.strategyV2=arr.includes(id)?arr.filter(x=>x!==id):[...arr,id];save();renderRevisionStudio();renderSnapshots();renderDesign();renderWorkflowStatuses();syncCompletionUI()});
  ...
  function renderCausal(){const h=$('#revisionCausalBuilder');if(!h)return;const c=state.revisionCausal,block=(title,key,defs)=>`<section><strong>${title}</strong><div class="choice-grid compact">${defs.map(([id,l])=>`<button data-causal="${key}" data-value="${id}" class="${c[key]===id?'active':''}">${l}</button>`).join('')}</div></section>`;h.innerHTML=block('1 · Hypothèse devenue fausse','hypothesis',revisionHypotheses)+block('2 · Information désormais menacée','information',revisionInfoOptions)+block('3 · Faiblesse restante après v2','weakness',revisionWeaknesses);h.querySelectorAll('[data-causal]').forEach(b=>b.onclick=()=>{state.revisionCausal[b.dataset.causal]=b.dataset.value;save();renderCausal();renderSnapshots();renderDesign()})}
  ...
  $('#finishBtn').onclick=()=>{if(!retrievalDefs.every(r=>r.opts.some(([id])=>id===state.retrieval[r.id])))return toast('Répondez aux cinq questions flash avant d’ouvrir la synthèse.');state.completed=true;save();$('#finalSynthesis').hidden=false;$('#finishBtn').hidden=true;renderStepper()};
