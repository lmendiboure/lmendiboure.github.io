(() => {
  ...
  host.innerHTML = cfg.sessions.map((s)=>{
    ...
    return `<article class="mission-card"><div class="mission-top"><span class="mission-number">${s.number}</span><span class="state-badge ${state==='locked'?'locked':''}">${stateLabels[state]}</span></div><div class="episode-meta"><span>${s.episode||''}</span><span>${s.timecode||''}</span></div><div class="mission-kicker">${s.kicker}</div><h3>${s.title}</h3><p class="mission-question">${s.question}</p>${s.story?`<div class="story-premise">${s.story}</div>`:''}<div class="episode-continuity"><div><span>DÉJÀ EN MAIN</span><strong>${s.inherits||'—'}</strong></div><div><span>À LIVRER</span><strong>${s.deliverable||'—'}</strong></div></div><div class="objective-row">${s.objectives.map(o=>`<span>${o}</span>`).join('')}</div>${progressHtml}${action}</article>`;
  }).join('');
