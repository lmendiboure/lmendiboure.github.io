(()=>{
  ...
  function stackVisual(v){
    ...
    return `<section class="projector-stack-visual">
      ...
      ${Array.isArray(v.patterns)&&v.patterns.length?`<div class="projector-pattern-grid">${v.patterns.map(x=>`<article><span>${esc(x.title||'')}</span><strong>${esc(x.flow||'')}</strong>${x.use?`<p>${esc(x.use)}</p>`:''}${x.watch?`<small>${esc(x.watch)}</small>`:''}</article>`).join('')}</div>`:''}
      ${v.table?`<div class="projector-reference-table-wrap"><table class="projector-reference-table"><thead><tr>${(v.table.columns||[]).map(x=>`<th>${esc(x)}</th>`).join('')}</tr></thead><tbody>${(v.table.rows||[]).map(r=>`<tr>${(r.cells||r||[]).map((x,n)=>`<${n===0?'th':'td'}>${esc(x)}</${n===0?'th':'td'}>`).join('')}</tr>`).join('')}</tbody></table>${v.table.note?`<p class="projector-table-note">${esc(v.table.note)}</p>`:''}</div>`:''}
      ${v.sidecar?`<div class="projector-sidecar"><span>${esc(v.sidecar.label||'OUTSIDE THE STACK')}</span><div><strong>${esc(v.sidecar.title||'')}</strong>${v.sidecar.text?`<p>${esc(v.sidecar.text)}</p>`:''}${Array.isArray(v.sidecar.items)&&v.sidecar.items.length?`<div class="sidecar-items">${v.sidecar.items.map(x=>`<b>${esc(x)}</b>`).join('')}</div>`:''}</div></div>`:''}
