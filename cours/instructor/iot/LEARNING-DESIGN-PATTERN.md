...
Use **open activity navigation with controlled answer disclosure**. Students may jump to any canonical activity without losing work, but reference answers, restitutions and formal conclusions should appear only after the corresponding task has been attempted or discussed.

Operational rule:

> **Navigation is open. Answers are staged. Meaningful commitments are versioned rather than silently overwritten.**

Implementation implications:

- keep saved progress distinct from the screen currently being viewed; do not use the saved frontier as a global navigation lock;
...
Do not use concept consolidation as a global navigation lock. If a downstream task depends on a concept, make that dependency visible in the prompt while keeping teacher-led and student review navigation open.

## 5. Mission dossier

The Mission dossier has two deliberately separate layers.
...
### 5.2 Cross-session decision record

When several sessions share the same case, persist only a **small structured record of decisions that materially define the engineered system**. Keep this separate from pedagogical state such as current screen, Unlocks, retrieval, hints or optional-depth progress.

Good cross-session fields are controlled decisions already produced by the activity, for example: architecture pattern, Top-3 requirements, one design stance, one unresolved assumption, or the event that forced a revision. Free text and drawings may remain in the student's export, but later sessions must **not parse them or depend on their quality**.

Every later session needs a complete generic fallback. The record personalises continuity; it must never become a prerequisite for teaching the session.
...
- [ ] Can students open any canonical activity without losing previous work?
