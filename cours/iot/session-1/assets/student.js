(() => {
  ...
  const technologyPracticeCases = [
    {id:'wearable',title:'Wearable → nearby phone',facts:'Small periodic data · battery device · phone normally nearby',options:['Bluetooth LE','Wi-Fi','LoRaWAN'],answer:'Bluetooth LE',why:'BLE is a natural fit for low-energy communication to a nearby phone or gateway.'},
    {id:'building',title:'Many constrained devices inside one building',facts:'Low-rate sensing/control · constrained nodes · local border router is acceptable',options:['IEEE 802.15.4','LoRaWAN','NB-IoT / LTE-M'],answer:'IEEE 802.15.4',why:'802.15.4 is a plausible low-power local radio/link foundation here, with higher networking layers still required above it.'},
    {id:'camera',title:'Powered camera in a connected building',facts:'Frequent images · mains power · managed WLAN already exists',options:['Wi-Fi','LoRaWAN','NB-IoT / LTE-M'],answer:'Wi-Fi',why:'Existing WLAN infrastructure and richer traffic make Wi-Fi the strongest fit among these options.'},
    {id:'rural',title:'Sparse battery sensors across a private site',facts:'Tiny report every 15 min · hundreds of metres to kilometres · gateways can be installed',options:['LoRaWAN','Wi-Fi','Bluetooth LE'],answer:'LoRaWAN',why:'Small infrequent traffic plus a private gateway deployment is a classic strong fit for LoRaWAN.'},
    {id:'meters',title:'Meters distributed across a city',facts:'Small messages · no local gateways to maintain · operator IoT coverage confirmed',options:['NB-IoT / LTE-M','Bluetooth LE','IEEE 802.15.4'],answer:'NB-IoT / LTE-M',why:'Confirmed operator coverage gives wide-area connectivity without maintaining local access infrastructure.'}
  ];
  ...
  function renderMysteries() {
    ...
    const next=$('#technologyAuditNext');if(next){next.disabled=completed<3;next.textContent=completed>=3?'Three cases compared → final campus decision':`Compare ${3-completed} more case${3-completed===1?'':'s'} →`;}
    renderStopSnapshots();
  }
  ...
  function renderStopSnapshots(){
    ...
    const t=$('#stopTechnologySnapshot');if(t){const quick=technologyPracticeCases.filter(c=>state.shapeChallenge?.[c.id]).map(c=>`<span class="snapshot-tech">${esc(c.title)} → ${esc(state.shapeChallenge[c.id])}</span>`).slice(0,3).join('');const dec=scenarioDefs.map(x=>{const st=state.scenarios[x.id]||{};return st.committed?`<div><strong>${esc(x.title)}</strong><span>${esc(st.choice||'')}</span>${st.twist?'<small>one fact changed</small>':''}</div>`:''}).filter(Boolean).join('');const cp=state.campusDecision?.position?campusDecisionPositions.find(x=>x[0]===state.campusDecision.position):null;const cu=state.campusDecision?.uncertainty?campusUncertainties.find(x=>x[0]===state.campusDecision.uncertainty):null;t.innerHTML=`<div class="snapshot-head"><strong>Our technology reasoning</strong><span>${technologyPracticeCases.filter(c=>state.shapeChallenge?.[c.id]).length} quick cases compared</span></div><div class="snapshot-tech-row">${quick||'<span class="snapshot-tech">No quick case recorded yet</span>'}</div><div class="snapshot-decisions">${dec||'<p class="empty-copy">No deployment decision committed yet.</p>'}</div>${cp?`<div class="snapshot-campus-return"><small>Back to campus</small><strong>${esc(cp[1])}</strong>${cu?`<span>Most decision-sensitive missing fact: ${esc(cu[1])}</span>`:''}</div>`:''}`;}
