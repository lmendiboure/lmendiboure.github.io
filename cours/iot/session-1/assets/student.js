(() => {
  ...
  const conceptDefs = {
    ...
    technology:{
      ...
      bridge:'The same service can favour different communication technologies when distance, traffic, device constraints or available infrastructure change.',
      formal:'Choose a technology <strong>after the flow requirements and deployment facts are visible</strong>. Then make the infrastructure that comes with that choice explicit: nearby peer, access point, private gateway or operator network.',
      ...
      fieldSummary:'Flow requirements + deployment facts → technology comparison → defended choice.',
      fieldKeep:'A technology choice can also imply infrastructure, ownership and dependencies. Comparable names need not describe the same technical scope.',
      tags:['requirements first','implicit infrastructure','validity domain']
    },
  };
  ...
  const challengeIds=['architecture','requirements'];
  ...
  function renderExpertProgress(){
    const n=challengeIds.filter(id=>state.challengeProgress?.[id]).length,total=challengeIds.length;
    const pill=$('#expertProgress'); if(pill){pill.textContent=`Depth trail ${n}/${total}`;pill.classList.toggle('active',n>0);}
    const finish=$('#expertFinish'); if(finish) finish.innerHTML=`<span>Optional depth trail</span><strong>${n} / ${total} explored</strong><small>Challenge routes use the same concepts with less guidance. They are optional and not graded.</small>`;
  }
  ...
  /* ---------- Navigation ---------- */
  const stepLabels = [
    ['1','IoT boundary'], ['2','Design baseline'], ['3','Monitoring → control'], ['4','Requirements'], ['5','Technology comparison'], ['6','Defend choice']
  ];
  ...
  const discoveryProbeUncertainties=[
    ['coverage','Actual Wi-Fi / operator / gateway coverage'],['traffic','Actual payload size + reporting cadence'],['power','Power source + maintenance target'],['layout','Distances, walls and outdoor obstacles'],['latency','End-to-end response / latency budget']
  ];
  function renderTechDiscovery(){
    const host=$('#techDiscoveryGrid'); if(!host)return;
    const use={
      ble:{vals:['Nearby','Light / moderate','Tight to moderate','Nearby phone / gateway'],fit:'Useful when a nearby phone, peer or gateway is normally present.'},
      wifi:{vals:['Local site','Moderate to high','Moderate / powered','Managed access points'],fit:'Useful when managed local coverage exists and traffic can be richer.'},
      dot154:{vals:['Local site','Low','Constrained','Border router + upper stack'],fit:'Useful for constrained local devices when a low-power local network is needed.'},
      lorawan:{vals:['Site / wide','Small, infrequent','Tight','Gateway(s) + network service'],fit:'Useful for small, infrequent data over larger distances when gateway infrastructure is acceptable.'},
      cellular:{vals:['Wide area','Small / moderate','Constrained to moderate','Operator coverage + service'],fit:'Useful when operator coverage exists and you do not want to deploy local access infrastructure.'}
    };
    host.innerHTML=discoveryFamilies.map((f,i)=>{const u=use[f.id];return `<article class="discovery-card revealed"><div class="discovery-top"><span class="discovery-icon">${f.icon}</span><div><span class="eyebrow">${esc(f.name)}</span><h3>${esc(f.title)}</h3></div></div><p>${esc(f.question)}</p><div class="shape-chain">${f.shape.map((x,j)=>`${j?'<span class="shape-arrow">→</span>':''}<span>${esc(x)}</span>`).join('')}</div><div class="shape-use-grid"><span><b>Distance / context</b>${u.vals[0]}</span><span><b>Traffic</b>${u.vals[1]}</span><span><b>Device budget</b>${u.vals[2]}</span><span><b>Infrastructure</b>${u.vals[3]}</span></div><div class="shape-fit"><b>Often useful when</b><span>${u.fit}</span></div></article>`}).join('');
    const board=$('#shapeReasoningBoard');
    if(board) board.innerHTML=`<div class="shape-reasoning-head"><div><span class="eyebrow">The rule to keep</span><h3>No family is “the best IoT technology”.</h3><p>Compare the deployment using the same questions every time: <strong>How far? How much data? What device constraints? What infrastructure already exists or can be deployed?</strong></p></div></div>`;
    const prog=$('#techDiscoveryProgress');if(prog)prog.textContent='5 families · same 4 criteria';
    const next=$('#discoveryNext');if(next){next.disabled=false;next.textContent='I have the map → try concrete cases';}
  }
  ...
  const technologyPracticeCases = [
    {id:'wearable',title:'Wearable → nearby phone',facts:'Small periodic data · battery device · phone normally nearby',options:['Wi-Fi','Bluetooth LE','LoRaWAN'],answer:'Bluetooth LE',why:'BLE is a natural fit for low-energy communication to a nearby phone or gateway.'},
    {id:'building',title:'Many constrained devices inside one building',facts:'Low-rate sensing/control · constrained nodes · local border router is acceptable',options:['LoRaWAN','IEEE 802.15.4','NB-IoT / LTE-M'],answer:'IEEE 802.15.4',why:'802.15.4 is a plausible low-power local radio/link foundation here, with higher networking layers still required above it.'},
    {id:'camera',title:'Powered camera in a connected building',facts:'Frequent images · mains power · managed WLAN already exists',options:['LoRaWAN','NB-IoT / LTE-M','Wi-Fi'],answer:'Wi-Fi',why:'Existing WLAN infrastructure and richer traffic make Wi-Fi the strongest fit among these options.'},
    {id:'rural',title:'Sparse battery sensors across a private site',facts:'Tiny report every 15 min · hundreds of metres to kilometres · gateways can be installed',options:['Bluetooth LE','LoRaWAN','Wi-Fi'],answer:'LoRaWAN',why:'Small infrequent traffic plus a private gateway deployment is a classic strong fit for LoRaWAN.'},
    {id:'meters',title:'Meters distributed across a city',facts:'Small messages · no local gateways to maintain · operator IoT coverage confirmed',options:['IEEE 802.15.4','Bluetooth LE','NB-IoT / LTE-M'],answer:'NB-IoT / LTE-M',why:'Confirmed operator coverage gives wide-area connectivity without maintaining local access infrastructure.'}
  ];

  function renderMysteries() {
    const host=$('#mysteryGrid'); if(!host)return;
    state.shapeChallenge=state.shapeChallenge||{};
    host.innerHTML=technologyPracticeCases.map(c=>{const choice=state.shapeChallenge[c.id];return `<article class="micro-game-card tech-case-card"><span class="eyebrow">Deployment case</span><h3>${esc(c.title)}</h3><p>${esc(c.facts)}</p><div class="micro-options">${c.options.map(o=>`<button type="button" class="micro-option ${choice===o?'active':''}" data-tech-case="${c.id}" data-choice="${esc(o)}">${esc(o)}</button>`).join('')}</div>${choice?`<div class="micro-feedback"><b>${choice===c.answer?'Strong fit among these options.':'A different option fits these facts more directly.'}</b>${esc(c.why)}</div>`:''}</article>`}).join('');
    host.querySelectorAll('[data-tech-case]').forEach(b=>b.addEventListener('click',()=>{state.shapeChallenge={...(state.shapeChallenge||{}),[b.dataset.techCase]:b.dataset.choice};saveState();renderMysteries();renderStopSnapshots();}));
    const completed=technologyPracticeCases.filter(c=>state.shapeChallenge[c.id]).length;
    const score=$('#mysteryScore');if(score)score.textContent=`${Math.min(completed,3)} / 3`;
    const next=$('#technologyAuditNext');if(next){next.disabled=false;next.textContent=completed>=3?'Three cases compared → campus decision':`${completed}/3 cases compared · continue anytime →`;}
    renderStopSnapshots();
  }
  ...
  const scenarioDefs = [
    ...
    {id:'meter', icon:'▤', title:'Meters distributed across a city', facts:['small messages','no local gateways to maintain','operator cellular-IoT coverage confirmed','long device lifetime'], options:['NB-IoT','Bluetooth LE','IEEE 802.15.4'], best:'NB-IoT', driver:2, why:'The crucial fact is confirmed operator LPWA coverage and no desire to deploy local access infrastructure. NB-IoT is therefore the strongest candidate among these choices.', twist:'Operator LPWA coverage is no longer available in part of the city.', twistImpact:'The strongest argument for NB-IoT has vanished. You may need local infrastructure, another operator technology, or a heterogeneous design.'},
  ...
  function renderScenarios() {
    ...
    host.innerHTML=scenarioDefs.map(s=>{
      ...
      return `<article class="scenario-card ${twist?'counterfactual-open':''}"><div class="scenario-top"><span class="scenario-icon">${s.icon}</span><div><div class="eyebrow">Deployment case</div><h3>${s.title}</h3></div></div><div class="fact-chip-row decision-driver">${s.facts.map((f,i)=>`<button type="button" class="fact-chip ${st.driver===i?'driver':''}" data-driver="${i}" data-sid="${s.id}" ${committed?'disabled':''}>${f}${st.driver===i?' · matters most':''}</button>`).join('')}</div><div class="scenario-options ${committed?'locked':''}">${s.options.map(o=>`<button type="button" class="scenario-option ${choice===o?'selected':''}" data-sid="${s.id}" data-choice="${esc(o)}" ${committed?'disabled':''}>${o}</button>`).join('')}</div>${!committed?`<p class="driver-hint">Choose a technology <b>and</b> the fact that matters most for your decision.</p>`:''}<div class="scenario-action-row">${!committed?`<button type="button" class="btn soft commit-scenario" data-sid="${s.id}" ${(choice&&Number.isInteger(st.driver))?'':'disabled'}>Commit decision</button>`:`<span class="decision-locked">Decision committed · ${esc(choice)}</span><button type="button" class="btn soft stress-scenario" data-stress-sid="${s.id}">${twist?'Changed fact opened':'Change one fact'}</button>`}</div>${committed?`<div class="scenario-rationale ${choice===s.best?'good':'challenge'}"><strong>${choice===s.best?'Strongest fit among these options.':'Worth discussing, but not the strongest fit here.'}</strong><span>${s.why}</span><small class="driver-feedback">You treated “${esc(s.facts[st.driver])}” as the most important fact. ${st.driver===s.driver?'That fact also carries a lot of weight in the reference reasoning.':`One plausible reference reading gives especially high weight to “${esc(s.facts[s.driver])}”. Your choice may still be defensible if you can explain the trade-off.`}</small></div>`:''}${twist?`<div class="counterfactual-box"><span class="eyebrow">WHAT IF THIS FACT CHANGES?</span><strong>${esc(s.twist)}</strong><p>${esc(s.twistImpact)}</p><div class="counterfactual-options"><button type="button" class="scenario-option ${st.twistVerdict==='keep'?'selected':''}" data-twist-verdict="keep" data-sid="${s.id}">Keep the original choice</button><button type="button" class="scenario-option ${st.twistVerdict==='reopen'?'selected':''}" data-twist-verdict="reopen" data-sid="${s.id}">Reopen the decision</button></div>${st.twistVerdict?`<small>${st.twistVerdict==='reopen'?'You recognised that a technology fit depends on deployment conditions.':'Be ready to defend why the changed fact does not invalidate the original fit.'}</small>`:''}</div>`:''}</article>`;
    }).join('');
  ...
  function updateScenarioGate(){const next=$('#scenarioNext');if(!next)return;const campus=state.campusDecision||{};next.disabled=false;next.textContent=(campus.position&&campus.uncertainty)?'Campus position recorded → final discussion':'Continue anytime → final discussion';}
  ...
  function renderCampusDecision(){
    ...
    host.innerHTML=`<div class="campus-position-grid">${campusDecisionPositions.map(([id,title,desc])=>`<button type="button" class="campus-position ${st.position===id?'active':''}" data-campus-position="${id}"><strong>${title}</strong><small>${desc}</small></button>`).join('')}</div>${st.position?`<div class="campus-uncertainty"><strong>Which missing fact could most change your answer?</strong><div class="challenge-chip-row">${campusUncertainties.map(([id,title])=>`<button type="button" class="chip-button ${st.uncertainty===id?'active':''}" data-campus-uncertainty="${id}">${title}</button>`).join('')}</div>${st.uncertainty?`<div class="campus-feedback"><b>Good engineering habit:</b> state the missing condition explicitly. ${campusUncertainties.find(x=>x[0]===st.uncertainty)?.[2]||''}</div>`:''}</div>`:''}`;
  ...
  function renderStopSnapshots(){
    ...
    const t=$('#stopTechnologySnapshot');if(t){const compared=technologyPracticeCases.filter(c=>state.shapeChallenge?.[c.id]);const quick=compared.map(c=>`<span class="snapshot-tech">${esc(c.title)} → ${esc(state.shapeChallenge[c.id])}</span>`).slice(0,3).join('');const cp=state.campusDecision?.position?campusDecisionPositions.find(x=>x[0]===state.campusDecision.position):null;const cu=state.campusDecision?.uncertainty?campusUncertainties.find(x=>x[0]===state.campusDecision.uncertainty):null;t.innerHTML=`<div class="snapshot-head"><strong>From comparison to campus decision</strong><span>${compared.length} case${compared.length===1?'':'s'} compared</span></div><div class="snapshot-tech-row">${quick||'<span class="snapshot-tech">No comparison recorded yet</span>'}</div>${cp?`<div class="snapshot-campus-return"><small>Campus position</small><strong>${esc(cp[1])}</strong>${cu?`<span>Missing fact that could change it: ${esc(cu[1])}</span>`:''}</div>`:'<p class="empty-copy">No campus position recorded yet — you can still revise it before the discussion.</p>'}`;}
